...
The runtime image includes modern stable `cargo` and `rustc` from the official Rust image so AgentLab Kubernetes jobs can execute Rust functional tests when Rust test changes are detected, for example:
